# build: docker build --no-cache --progress=plain --target binary --build-arg NGINX_VERSION=1.29.3 -t ghcr.io/tob1as/static-nginx:base-debian -f static-nginx.base-debian.Dockerfile .
...
LABEL org.opencontainers.image.title="Static NGINX"\
      ...
      org.opencontainers.image.description="Static NGINX${NGINX_VERSION:+ ${NGINX_VERSION}} build with pcre2${PCRE2_VERSION:+-${PCRE2_VERSION}}, zlib${ZLIB_VERSION:+-${ZLIB_VERSION}} and openssl${OPENSSL_VERSION:+-${OPENSSL_VERSION}} on Debian" \
...
RUN VERSION_FILE="/tmp/versions.txt" ; \
    touch ${VERSION_FILE} ; \
    echo "NGINX_VERSION=${NGINX_VERSION}" >> ${VERSION_FILE} ; \
    echo "PCRE2_VERSION=${PCRE2_VERSION}" >> ${VERSION_FILE} ; \
    echo "ZLIB_VERSION=${ZLIB_VERSION}" >> ${VERSION_FILE} ; \
    echo "OPENSSL_VERSION=${OPENSSL_VERSION}" >> ${VERSION_FILE} ; \
    cat ${VERSION_FILE}
...
LABEL org.opencontainers.image.title="Static NGINX" \
      ...
      org.opencontainers.image.revision="${VCS_REF}" \
      org.opencontainers.image.description="Baseimage with Static NGINX${NGINX_VERSION:+ ${NGINX_VERSION}} build with pcre2${PCRE2_VERSION:+-${PCRE2_VERSION}}, zlib${ZLIB_VERSION:+-${ZLIB_VERSION}} and openssl${OPENSSL_VERSION:+-${OPENSSL_VERSION}} on Debian" \
